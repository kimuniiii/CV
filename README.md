...
基本的には「サブスクリプション動画配信サービスの大規模新規開発」と同様のコードを使用して実装を行ったが、React のバージョンが18になったことでレンダリングの仕組みが、`ReactDOM.render()` の代わりに `createRoot()` を使用するようになった関係でバージョン差異によるエラーを解決するために調査を実施する必要があったが、その調査を積極的に実施し。チームのメンバーにナレッジを共有して、React のバージョン18に対応できるような共通のHooksなどを実装することで社内の生産性向上に大きく貢献した。
